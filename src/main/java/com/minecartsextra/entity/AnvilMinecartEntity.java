...
public class AnvilMinecartEntity extends AbstractMinecart implements ILeashedMinecart {
    private static final EntityDataAccessor<Integer> LEASH_HOLDER_ID = SynchedEntityData.defineId(AnvilMinecartEntity.class, EntityDataSerializers.INT);

    public AnvilMinecartEntity(EntityType<? extends AnvilMinecartEntity> type, Level level) {
        super(type, level);
    }

    public AnvilMinecartEntity(Level level, double x, double y, double z) {
        super(ModEntities.ANVIL_MINECART.get(), level, x, y, z);
    }
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        if (!this.level().isClientSide) {
            player.openMenu(new SimpleMenuProvider((id, inventory, p) -> 
                new AnvilMenu(id, inventory, ContainerLevelAccess.create(this.level(), this.blockPosition())) {
                    @Override
    ...
    public void destroy(DamageSource source) {
        ...
        if (this.level().getGameRules().getBoolean(net.minecraft.world.level.GameRules.RULE_DOENTITYDROPS)) {
            ItemStack itemstack = new ItemStack(ModItems.ANVIL_MINECART.get());
    ...
    protected Item getDropItem() {
        return ModItems.ANVIL_MINECART.get();
    }
}
